...
class BrakesFailure:
    ...
    def __init__(self, name, fixtime, garage, stock_number, propability, speed_reduction, speed_reduction_night, failure_deterioration, next_failure, fuel_penalty, chance_of_repair_failure, brake_threshold) -> None:
        self.name = name
    ...
    @staticmethod
    def load_from_file(filename = "./data/brakes_failures.json"):
        with open(filename, 'r') as file:
            data = json.load(file)
            failures = []
            for item in data:
                failure = BrakesFailure(
                    name=item["name"],
